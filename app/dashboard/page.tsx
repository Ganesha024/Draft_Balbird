        ...
        {quickStats.map((stat, index) => (
          <Card key={index} className="p-4 hover:shadow-md transition-shadow">
            <div className="flex items-center justify-between mb-3">
              ...
              <span className={`text-xs font-medium ${stat.change.startsWith('+') ? 'text-green-600' : stat.change.startsWith('-') ? 'text-red-600' : 'text-slate-400'}`}>
                {stat.change}
              </span>
            </div>
      ...
      <Card className="p-5 mb-6">
        ...
        <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {operationalVerticals.map((v, idx) => (
            <div key={idx} className="flex items-center gap-3 p-3 rounded-xl bg-white/5/5 border border-white/10">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        <Card className="p-5">
          ...
          <div className="mt-4 space-y-3">
            {recentActivities.map(activity => (
              <div key={activity.id} className="flex items-start gap-3 p-3 rounded-xl bg-white/5/5 border border-white/10">
        ...
        <Card className="p-5">
          ...
          <div className="mt-4 grid grid-cols-2 gap-3">
            ...
            <Button className="h-auto p-4 flex flex-col gap-2 rounded-xl bg-white/5/10 text-white hover:bg-white/5/20">
      ...
      <Card className="p-5">
        ...
        <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="p-4 rounded-xl bg-accent/5 border border-accent/20">
            ...
            <p className="text-sm text-slate-400 leading-relaxed">AI-assisted capability matching now considers certification compatibility and geographic proximity.</p>
            <p className="text-xs text-slate-400 mt-3 font-medium">2 days ago</p>
          </div>
          <div className="p-4 rounded-xl bg-white/5/5 border border-white/10">
            ...
            <p className="text-sm text-slate-400 leading-relaxed">End-to-end documentation flow now supports IATF 16949 and AS9100 compliance templates.</p>
            <p className="text-xs text-slate-400 mt-3 font-medium">5 days ago</p>
          </div>
        </div>
      </Card>
